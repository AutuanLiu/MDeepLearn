...
plt.figure(2)
for label, col in zip([0, 1, 2], colors):
    plt.scatter(X_resample[y_resample == label, 0], X_resample[y_resample == label, 1],
                color=col, label='class' + str(label))
plt.title('resample data')
plt.legend(loc='best')

# 创建model实例, 4 个模型
model_name = ['GradientBoostingClassifier', 'AdaBoostClassifier',
              'XGBClassifier', 'RandomForestClassifier'
              ]

model_instance = [GradientBoostingClassifier(), AdaBoostClassifier(),
