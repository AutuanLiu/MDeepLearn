...
-------------------------------------------------
   ...
   Description :  实现 lasso 回归
   和普通回归算法不同的是在损失函数上添加了 正则项(系数的某一个倍数)
   L1 正则项
...
# 这里的定义比较简单
# 限制斜率的系数不超过 0.7
param = tf.constant(0.7)
regular = tf.multiply(A, param)
loss = tf.reduce_mean(tf.square(target - mod)) + regular
